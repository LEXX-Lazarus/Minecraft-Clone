...
void DebugOverlay::render(int windowWidth, int windowHeight,
    ...
    std::string posText = "Position: " + std::to_string((int)posX) + ", " +
        std::to_string((int)posY) + ", " + std::to_string((int)(-posZ));  // NEGATE posZ HERE
    std::string yawText = "Yaw: " + std::to_string((int)yaw);  // ADD THIS
    std::string dirText = "Direction: " + getCardinalDirection(yaw);
    std::string fpsText = "FPS: " + std::to_string((int)fps);

    renderText(posText, 10, 50, 1.2f, windowWidth, windowHeight);
    renderText(yawText, 10, 80, 1.2f, windowWidth, windowHeight);   // ADD THIS
    renderText(dirText, 10, 110, 1.2f, windowWidth, windowHeight);  // MOVED DOWN
    renderText(fpsText, 10, 140, 1.2f, windowWidth, windowHeight);  // MOVED DOWN
