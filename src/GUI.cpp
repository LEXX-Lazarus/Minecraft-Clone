...
const char* guiTexturedVertexShader = R"(
#version 330 core
layout (location = 0) in vec2 aPos;
...
const char* guiTexturedFragmentShader = R"(
#version 330 core
out vec4 FragColor;
...
GUI::GUI() : VAO(0), VBO(0), shaderProgram(0), texturedShaderProgram(0), fullscreenButtonTexture(0), resumeButtonTexture(0) {
}
...
void GUI::initialize() {
    ...
    texturedShaderProgram = createTexturedGUIShader();
...
unsigned int GUI::createTexturedGUIShader() {
    // Compile vertex shader
    unsigned int vertexShader = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(vertexShader, 1, &guiTexturedVertexShader, nullptr);
    glCompileShader(vertexShader);
    ...
    if (!success) {
        char infoLog[512];
        glGetShaderInfoLog(vertexShader, 512, nullptr, infoLog);
        std::cerr << "GUI Textured Vertex shader compilation failed:\n" << infoLog << std::endl;
    }

    // Compile fragment shader
    unsigned int fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fragmentShader, 1, &guiTexturedFragmentShader, nullptr);
    glCompileShader(fragmentShader);
    ...
    if (!success) {
        char infoLog[512];
        glGetShaderInfoLog(fragmentShader, 512, nullptr, infoLog);
        std::cerr << "GUI Textured Fragment shader compilation failed:\n" << infoLog << std::endl;
    }
    ...
    if (!success) {
        char infoLog[512];
        glGetProgramInfoLog(program, 512, nullptr, infoLog);
        std::cerr << "GUI Textured Shader program linking failed:\n" << infoLog << std::endl;
    }
...
unsigned int GUI::loadButtonTexture(const char* path) {
    ...
    if (data) {
        ...
        std::cout << "GUI Button texture loaded: " << path << std::endl;
    }
    else {
        std::cerr << "Failed to load GUI button texture: " << path << std::endl;
    }
...
void GUI::renderPauseMenu(int windowWidth, int windowHeight) {
    ...
    // Update attribute pointers for textured format
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)(2 * sizeof(float)));
    glEnableVertexAttribArray(1);

    // Resume button (textured)
    ...
    // Restore attribute pointers for color format (in case needed later)
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(2 * sizeof(float)));
    glEnableVertexAttribArray(1);
